...
class SearchController extends AbstractController
{
    ...
    public function SearchGame(Request $request,GameRepository $jeuxRepository , CartService $cartService)
    {
        ...
        $form = $this->createFormBuilder($recherche)
            ...
            if ($form->isSubmitted() && $form->isValid()) { // $form->getData() holds the submitted values
                $recherche = $form->getData();
                dump($form->getData()->getSearch()); 
                $jeux = $jeuxRepository->trouverJeux($recherche->getSearch());
                if (!$jeux) {
                    return $this->render('liste_des_jeux/index.html.twig',['form' => $form->createView(),'total' => $cartService->getTotal(), 'itemsTotal' => $cartService->getTotalItem(), 'allgames' => $jeux, 'error' => ' Nous avons trouver aucun jeux correspondant à votre recherche!']);
                }
                ...
                ///////////////////////// L'AFFICHAGE AVANT ENVOI DE LA RECHERCHE ////////////////
